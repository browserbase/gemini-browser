...
>
  Check out the [Stagehand Evals](https://www.stagehand.dev/evals) to see
  different LLMs compare in Stagehand.
</Card>
...
<Tabs>
  <Tab title="TypeScript">
    You can set `logInferenceToFile: true` in the Stagehand constructors. This
    will dump all act, extract, and observe calls to a directory called
    `inference_summary`.
  </Tab>
  <Tab title="Python">
    You can set `log_inference_to_file=True` in the Stagehand config. This will
    dump all act, extract, and observe calls to a directory called
    `inference_summary`.
  </Tab>
</Tabs>
...
<Tip>
  To run evals, you'll need to clone the [Stagehand
  repo](https://github.com/browserbase/stagehand) and run `npm install` to
...
<Tip>
  To run LLM evals, you'll need a [Braintrust
  account](https://www.braintrust.dev/docs/).
</Tip>
