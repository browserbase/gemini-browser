...
<Tip>
  You can pass an `ObserveResult` to `act()` to perform the suggested action,
  which will yield a faster and cheaper result (no LLM inference).
</Tip>
...
<ParamField path="variables" type="Record<string, string>" optional>
  Variables to use in the action. Variables in the action string are referenced
  using %variable_name%
</ParamField>
