...
<ParamField path="model" type="string" required>
  Specifies the LLM model. Currently supporting: - **OpenAI**:
  `computer-use-preview` - **Anthropic**: `claude-sonnet-4-20250514` and
...
<ParamField path="provider" type="string" optional>
  Specifies the LLM provider. Currently supporting: - **OpenAI**: `openai` -
  **Anthropic**: `anthropic`
</ParamField>
...
<ParamField path="completed" type="boolean">
  Whether the agent fully completed its task (can be false even if success is
  true)
</ParamField>
