...
/\*_ The observe result will be an object with the following shape:
{
...
<CodeGroup>
  ```typescript TypeScript const formValues = await page.observe("get the text
  inputs on the page"); ``` ```python Python form_values = await
  page.observe("get the text inputs on the page") ```
</CodeGroup>
