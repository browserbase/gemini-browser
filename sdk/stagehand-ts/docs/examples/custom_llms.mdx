...
<Tip>
  Small models on **Ollama** are really difficult to get consistent structured
  outputs from. Though these models are "supported" via an OpenAI-compatible
  API, we do not recommend them yet for Stagehand.
</Tip>
...
<Expandable title="Supported Providers">
  - **OpenAI** - **Anthropic** - **Google** - **xAI** - **Azure** - **Groq** -
  **Cerebras** - **TogetherAI** - **Mistral** - **DeepSeek** - **Perplexity** -
  **Ollama**
</Expandable>
...
<Note>Custom LLMs are currently only supported in TypeScript.</Note>
...
const stagehand = new Stagehand({
  llmClient: new LangchainClient(new ChatOpenAI({ model: "gpt-4o" })),
});
```
