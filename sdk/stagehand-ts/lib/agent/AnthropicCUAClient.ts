...
import { compressAnthropicConversationImages } from "./utils/imageCompression";
    ...
    try {
      ...
      if (name === "computer") {
        ...
        if (action === "screenshot") {
          ...
          const key =
            action === "key" ? (input.text as string) : (input.keys as string);
          const keys = key.split("+");
          return {
            type: "keypress",
            keys: keys.map((key) => mapKeyToPlaywright(key)),
            ...input,
          };
